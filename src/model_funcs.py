...
class MultinomialExpectationMaximizer:
    def __init__(self, K, rtol=1e-4, max_iter=20, restarts=10, seed = 12):
        self._K = K
        self._rtol = rtol
    ...
    def _m_step(self, Y, tau, Z):
        ...
        """
        # indices = np.random.choice(Y.shape[0], self._batch_size, replace=False)
        # Y_subset = Y[indices]
        # tau_subset = tau[indices]

        # pi = tau_subset.sum(axis=0) / tau_subset.sum()
        # weighted_counts = tau_subset.T.dot(Y_subset)
        ...
        #         numerator = np.sum((Z == l) * Y[:, k-1].reshape(-1, 1))
        #         denominator = np.sum((Z == l) * Y.sum(axis=1).reshape(-1, 1))
        #         if denominator != 0:
        #             theta[l-1, k-1] = numerator / denominator
        #         else:
        #             theta[l-1, k-1] = 0
    ...
    def _train_once(self, Y):
        ...
        for it in range(self._max_iter):
            ...
            #     break
        # take the mean of all values after a burn in period of 100 iterations
        pi_est = np.mean(pi_list[100:], axis=0)
        tau_est = np.mean(tau_list[100:], axis=0)
        theta_est = np.mean(theta_list[100:], axis=0)
        loss = self._compute_loss(Y, pi_est, theta_est, tau_est)

        return pi_est, theta_est, tau_est, loss, theta_list, pi_list, tau_list, Z_list
        ...
        #     if it > 0 and (np.abs((prev_loss - loss) / prev_loss) < self._rtol):
        #         break
        # return pi, theta, tau, loss, theta_list
    ...
    def fit(self, Y):
        ...
        for it in range(self._restarts):
            pi, theta, tau, loss, theta_list, pi_list, tau_list, Z_list = self._train_once(Y)
            if loss > best_loss:
                best_loss = loss
                best_pi = pi
                best_theta = theta
        ...
        return best_loss, best_pi, best_theta, best_tau, best_theta_list, best_pi_list, best_tau_list, best_Z_list


def run_em(Y, K=10, max_iter=1000, restarts=3, seed = 12):
    """
    ...
    model = MultinomialExpectationMaximizer(K, restarts=restarts, max_iter=max_iter, seed = seed)
    _, pi, theta, tau, theta_list, pi_list, tau_list, Z_list  = model.fit(Y)
    ...
    return likelihoods, best_pi, best_theta, best_tau, best_theta_list, best_pi_list, best_tau_list, best_Z_list
